...
function generate_new_detail($colour, $animal, $number)
{
    $name_1 = $colour . $_SESSION['username'];
    $name_2 = $_SESSION['username'] . "_is_a_" . $animal;
...
<section id="content">
    ...
    <form action='update_page.php' method='post'>
        Fav. Colour: <input type='text' name='colour'><br>
        Fav. Animal: <input type='text' name='animal'><br>
        Fav. Number: <input type='number' name='number'><br>
        <input class="detailsButton" type='submit' value='Update my details'>
    </form>

    <?php
    if (isset($_POST['colour']) && isset($_POST['animal']) && isset($_POST['number'])) {
        $some_names = generate_new_detail($_POST['colour'], $_POST['animal'], $_POST['number']);
        $count = 1;
